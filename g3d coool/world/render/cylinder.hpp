...
{
	float y = (face * (height + 0.01f));

	/* vertical */
	add_plusss_quad_relative(self, where,
		Vector3(0.1f, y, face * (-height * 0.75f)),
		Vector3(-0.1f,y, face * (-height * 0.75f)),
		Vector3(-0.1f,y, face * (height * 0.75f)),
		Vector3(0.1f, y, face * (height * 0.75f)), Color3::white());

	/* horizontal */
	add_plusss_quad_relative(self, where,
		Vector3(height * 0.75f, y, face * (-0.1f)),
		Vector3(height * 0.05f, y, face * (-0.1f)),
		Vector3(height * 0.05f, y, face * (0.1f)),
		Vector3(height * 0.75f, y, face * (0.1f)), Color3::white());
...
static void build_circle(block::Block* self, const CoordinateFrame& where, int triangles, float radius, float y, bool invertFace, const Color4& color)
{
	...
	for (int i = 0; i < triangles - 2; i++) {
		...
		Vector3 v1 = circleVertices[i + 1 + invertFace];
		Vector3 v2 = circleVertices[i + 2 - invertFace];
...
static void build_cylinder(block::Block* self, const CoordinateFrame& where, const Color4& color, float height, float radius, int slices)
{
	...
	for (int o = 0; o < 4; o++)
	{
		...
		for (int i = 0; i < slices; i++)
		{
			...
			Vector3 v0 = Vector3(radius * x1, p + h, radius * z1);
			Vector3 v1 = Vector3(radius * x2, p + h, radius * z2);
			Vector3 v2 = Vector3(radius * x2, p, radius * z2);
			Vector3 v3 = Vector3(radius * x1, p, radius * z1);

			add_block_vertex(self, v0, where.pointToWorldSpace(v0));
			add_block_vertex(self, v1, where.pointToWorldSpace(v1));
			add_block_vertex(self, v2, where.pointToWorldSpace(v2));

			add_block_vertex(self, v2, where.pointToWorldSpace(v2));
			add_block_vertex(self, v3, where.pointToWorldSpace(v3));
			add_block_vertex(self, v0, where.pointToWorldSpace(v0));
		}
	}
